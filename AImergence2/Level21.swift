...
class Level21 : Level5 {
    ...
    override func play(_ localExperiment: Experiment) -> (Experience, Int) {
        var result = 0
        if localExperiment.number == remoteExperimentNumber {
            result = 1
        }
        remoteExperimentNumber = nil
        let experience = experiences[localExperiment.number][result]
